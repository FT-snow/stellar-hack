    ...
    >
      ...
      <div className="max-w-2xl">
        {awards.map((award, i) => (
          <motion.div
            key={award}
            className="flex items-center gap-6 py-5 border-b border-white/[0.04] group cursor-default"
            initial={{ opacity: 0, x: -12 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, margin: '-50px' }}
            transition={{ duration: 0.4, delay: i * 0.06, ease: 'easeOut' }}
          >
            <span className="font-mono text-[10px] tracking-wider w-8" style={{ color: 'rgba(255,255,255,0.15)' }}>
